...
对于任意的节点 ![[公式]](https://www.zhihu.com/equation?tex=v_i+%5Cin+G) :
...
![WL test一次迭代结果](/static/RelationalPooling/Hexane-2D.png)
...
![WL test一次迭代结果](/static/RelationalPooling/CycloHexane-2D.png)
...
![WL test一次迭代结果](/static/RelationalPooling/CSL_graph.png)
...
![WL test一次迭代结果](/static/RelationalPooling/gnn_example.png)

然而，由于图所具有的特殊性，其没有严格的序列关系，其结点顺序改变也会引起近邻矩阵$\boldsymbol A$的改变：

![WL test一次迭代结果](/static/RelationalPooling/featureless_PI.png)

上图为交换了1和2编号的图的近邻矩阵的变化(交换1、2行和1、2列)。而对于具有结点特征的图，还需要对其结点特征矩阵$\boldsymbol X$进行行变换，如下图中对$H_2O$分子的结点编号变换：

![WL test一次迭代结果](/static/RelationalPooling/withfeature_PI.png)
...
![WL test一次迭代结果](/static/RelationalPooling/unique_id_CSL.png)
...
![WL test一次迭代结果](/static/RelationalPooling/RP_GNN_example.png)
...
![WL test一次迭代结果](/static/RelationalPooling/pi_sgd_example.png)

（2）$K-ary$子图

​	对具有$n$个结点的图，只取其k个结点作为子图，可以结合$\pi-SGD$方法进行模型训练。如图，对一次排序$\pi(1,2,3,4,5) = (3,4,1,2,5)$，取$k=3$的子图。

![WL test一次迭代结果](/static/RelationalPooling/k-ary-example.png)
...
![WL test一次迭代结果](/static/RelationalPooling/experiment1_result.png)

#### 4.2 实验二 分子结构预测

数据集介绍：

![WL test一次迭代结果](/static/RelationalPooling/experiment2_datasets.png)

实验结果：

![WL test一次迭代结果](/static/RelationalPooling/experiment2_result.png)
